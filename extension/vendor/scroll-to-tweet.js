...
(() => {
  ...
  window.scrollToTweet = event => {
    ...
    const calculatedNavHeight =
      $("header").clientHeight +
      ($("header").clientHeight - $("nav").clientHeight);

    // this padding has to be applied to factor in the padding that's added to the container
    const tweetsContainerPadding = Math.ceil(
      parseInt($("._1nQuzuNK").style.paddingTop.replace("px", ""), 10)
    );
    ...
    const totalOffset = tweetOffset =>
      tweetOffset +
      calculatedNavHeight +
      tweetsContainerPadding +
      offsetToNotShowTweetTopBorder;

    // takes a tweet's offsetTop and checks if it's below the navigation bar
    const tweetIsBelowNav = offset =>
      totalOffset(offset) > currentTop + bufferTop;
